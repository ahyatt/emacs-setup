...
Objed is interesting & fits in well with existing emacs conventions. I’m trying
it out. Frankly, it confuses me right now, but maybe I’ll get use to it.
...
Multiple cursors is very easy to use. There’s a few similar packages, but this one seems most flexible.
#+BEGIN_SRC emacs-lisp
  (use-package multiple-cursors)
#+END_SRC

Magit is essential for git. Unfortunately, I don't know how to install it - it
requires a specific version of git-commit that isn't available.
#+BEGIN_SRC emacs-lisp
  (use-package magit
