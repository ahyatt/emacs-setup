...
It's useful to have methods of getting the URLs (for Mac only).  If this doesn't work, you may have to execute this within emacs (for example, in eshell): =osascript -e 'tell application "Chrome" to return title of front window'= to force the permission.
...
Use BBDB as the address book for Gnus.  Add recipients to BBDB when messages
are sent, and use those contacts with the standard completion-at-point UI.
Ignore Substack's generated =reply+= addresses and GitHub's shared
=notifications@github.com= address.  The former changes continually, while the
latter is reused with unrelated sender names, so neither is a useful contact.
#+begin_src emacs-lisp
(use-package bbdb
  :custom
  (bbdb-file "~/.bbdb")
...
#+end_src
*** Gnus
It's also worth it to make sure gnus works well:
