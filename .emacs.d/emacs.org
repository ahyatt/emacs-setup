...
** Bugfixes for org 
Fix for issue where scheduling something in the agenda leads to an infinite
loop.  It's an interaction between scheduling and the invisible blocked tasks
...
*** org-present                                                :experimental:
Could this be an effective way to present?  Let's take a look.
#+BEGIN_SRC emacs-lisp
